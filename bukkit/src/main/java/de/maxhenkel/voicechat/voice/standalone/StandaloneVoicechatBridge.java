...
public class StandaloneVoicechatBridge {
    ...
    public void start() {
        ...
        scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "VoiceChatStandaloneSync");
            t.setDaemon(true);
            return t;
        });
        scheduler.scheduleAtFixedRate(this::queuePlayerUpdates, 0L,
                Voicechat.SERVER_CONFIG.standaloneUpdateInterval.get(), TimeUnit.MILLISECONDS);
    ...
    private void queueFullSync() {
        ...
        Voicechat.compatibility.runTask(() -> {
            for (Player player : Bukkit.getOnlinePlayers()) {
                queueStateAndPosition(player, voiceServer);
                if (voiceServer.hasSecret(player.getUniqueId())) {
                    Secret secret = voiceServer.getSecret(player.getUniqueId());
                    enqueue(message(MSG_PLAYER_SECRET, out -> {
                        writeUUID(out, player.getUniqueId());
                        out.write(secret.getSecret());
                    }));
                }
            }
    ...
    private void queueStateAndPosition(Player player, Server voiceServer) {
        PlayerState state = voiceServer.getPlayerStateManager().getState(player.getUniqueId());
        if (state == null) {
